...
export function GameRoomPage({ roomId, apiClient }: { roomId: string; apiClient: ApiClient }) {
  ...
  return <main className="game-layout"><header><Link to="/">☾ Werewolf Arena</Link><ConnectionStatus status={session.connection} onRefresh={() => void session.refresh()} onContinue={() => void session.continueRoom()} /></header><div className="game-columns"><RoomTimeline events={session.events} participants={session.snapshot.participants} /><aside className="room-sidebar">{session.snapshot.view_mode === "active" && <ActionPanel state={session.snapshot} onSubmit={submit} pending={pending} />}{session.snapshot.view_mode === "active" && <PrivatePanel state={session.snapshot} />}{session.snapshot.view_mode === "spectating" && <SpectatorPanel />}{session.snapshot.view_mode === "finished" && (report ? <FinishedReport report={report} /> : <p>正在生成完整复盘…</p>)}<ModelHealthPanel health={session.snapshot.agent_health} /><PlayerRail state={session.snapshot} />{session.error && <p role="alert">{session.error}</p>}{session.snapshot.status === "finished" && <Link className="restart" to="/">返回历史</Link>}</aside></div></main>;
}
