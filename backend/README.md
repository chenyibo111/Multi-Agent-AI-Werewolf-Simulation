...
- React 浏览器对局界面、仅存储房间 ID 的本地历史、死亡旁观、终局安全复盘和房间删除。

不会向 HTTP 或 WebSocket 暴露完整权威 `GameState`、进行中其他玩家身份、服务端事件、原始模型输出或思维链。玩家死亡后只会收到公开状态与公开事件；身份只有在终局的专用复盘接口中揭示。未配置模型时，服务可用于离线规则/API 验证，但 AI 不会请求外部服务。
